...
app.post("/ai-evaluate-pdf", upload.single("report"), async (req, res) => {
  try {
    ...
    const reportName = req.body.reportName?.trim();

    // ✅ STEP 2: NAME CHECK (INPUT vs REGISTERED)
    if (!reportName || reportName.toLowerCase() !== user.name.toLowerCase()) {
      return res.json({
        success: false,
        message: "Upload the correct person's name",
      });
    }
    ...
    console.log("PDF TEXT SAMPLE:", parsed.text.slice(0, 300));
    console.log("======== DEBUG END ========");
    
    if (!parsed.text || parsed.text.length < 10) {
      console.log("⚠️ Low text, but continuing...");
    }

    // NEW (flexible ✅)
    const nameParts = user.name.toLowerCase().split(" ");

    const match = nameParts.some(part => lowerText.includes(part));
